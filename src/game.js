...
export class Game {
  ...
  updateTurbulence(dt) {
    ...
    switch (T.state) {
      ...
      case 'warning': {
        ...
        this.notice = {
          text: T.isFinal
            ...
            : `⚠ Turbulence in ${Math.ceil(left)}s — seat your passengers!`,
        }
        if (left <= 0) {
          T.state = 'active'
      ...
      case 'active': {
        this.notice = { text: T.isFinal ? '🛬 FINAL DESCENT — HOLD ON!' : '✈ TURBULENCE — hold on to something!' }
  ...
  trySpawnRequest() {
    ...
    const candidates = this.props.passengers.filter(
      (p) => !busy.has(p) && (p.userData.requestCooldown ?? 0) <= this.elapsed,
    )
